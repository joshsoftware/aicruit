                ...
                <td className="px-6 py-2 text-sm text-black-tertiary">
                  <div className="flex items-center gap-2">
                    <div className="w-full h-2 rounded bg-gray-200">
                      {(() => {
                        const raw = Number(resume.matching_score || 0);
                        const score10 = isNaN(raw) ? 0 : Math.max(0, Math.min(10, raw));
                        const colorClass = score10 >= 7.5 ? "bg-green-500" : score10 >= 5 ? "bg-yellow-500" : "bg-red-500";
                        return (
                          <div
                            className={`h-full rounded transition-all duration-300 ${colorClass}`}
                            style={{ width: `calc(${score10} * 10%)` }}
                          />
                        );
                      })()}
                    </div>
